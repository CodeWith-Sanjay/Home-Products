...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        const currentOrder = await pool.query("SELECT order_status FROM orders WHERE order_id = $1", [order_id]);
        ...
        await pool.query(
            "UPDATE orders SET order_status = $1, cancellation_reason = $2, updated_at = NOW() WHERE order_id = $3",
            [status, status === 'Cancelled' ? notes : null, order_id]
        );

        // 2. Automatically update seller payouts if order is delivered
        if (status === 'Delivered') {
            // Pay the sellers
            await pool.query(
                "UPDATE seller_commissions SET status = 'Paid' WHERE order_id = $1",
                [order_id]
            );
            // Mark Customer Payment as Paid (For Cash On Delivery orders)
            await pool.query(
                "UPDATE payments SET payment_status = 'Paid', paid_at = NOW() WHERE order_id = $1 AND payment_method = 'cod'",
                [order_id]
            );
            ...
            // Restore Stock
            const orderItems = await pool.query("SELECT product_id, variant_id, quantity FROM order_items WHERE order_id = $1", [order_id]);
            for (const item of orderItems.rows) {
                if (item.variant_id) {
                    await pool.query("UPDATE product_variants SET stock_quantity = stock_quantity + $1 WHERE variant_id = $2", [item.quantity, item.variant_id]);
                } else {
            ...
            const orderInfo = await pool.query("SELECT DATE(placed_at) as date FROM orders WHERE order_id = $1", [order_id]);
            if (orderInfo.rows.length > 0) {
                const sellerSubtotals = await pool.query("SELECT seller_id, seller_subtotal FROM order_sellers WHERE order_id = $1", [order_id]);
        ...
        const customerIdRes = await pool.query("SELECT customer_id FROM orders WHERE order_id = $1", [order_id]);
